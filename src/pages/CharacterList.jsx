...
const PERSONALITY_INFO = {
  friendly: { emoji: '😊', color: 'neon-green' },
  sarcastic: { emoji: '😏', color: 'neon-yellow' },
  wise: { emoji: '🧙', color: 'neon-cyan' },
  dark: { emoji: '🌑', color: 'neon-purple' },
  cheerful: { emoji: '🌟', color: 'neon-pink' },
};
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                    <span className="text-4xl sm:text-6xl group-hover:scale-110 transition-transform flex-shrink-0">{character.avatar}</span>
                    <div className="min-w-0 flex-1">
                      <h3 className={`text-lg sm:text-2xl font-bold text-${personalityInfo.color} transition-colors truncate`}>
                        {character.name}
                      </h3>
                      <div className="flex items-center space-x-2 mt-1">
                        <span className="text-base sm:text-lg">{personalityInfo.emoji}</span>
                ...
                <div className="flex items-center space-x-3 sm:space-x-4 text-xs text-white/50 mb-5 pb-5 border-b border-white/10 font-mono">
                  <span className="flex items-center">💬 {messageCount}</span>
                  <span className="hidden sm:inline">📅 {new Date(character.createdAt).toLocaleDateString()}</span>
                </div>
      ...
      <Modal isOpen={showImportModal} onClose={() => setShowImportModal(false)} title="Import Data">
        <div className="space-y-4">
          ...
          <div className="bg-neon-yellow/10 border border-neon-yellow/30 p-4 rounded-lg">
            <p className="text-sm text-neon-yellow font-medium">⚠️ Warning: This will overwrite all existing characters and conversations!</p>
          </div>
          <textarea
            value={importText}
